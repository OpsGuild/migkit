...
CHANGELOG_DIR="test/changelog-xml"
SCHEMA_DIR="test/schema-xml"
TEST_RESULTS_DIR="test/results-xml"
...
test_liquibase_init() {
    ...
    # Test --init command
    if docker compose -f test/docker-compose.yaml run --rm -e LIQ_DB_HOST=postgres-test -e LIQ_DB_USER=testuser -e LIQ_DB_PASSWORD=testpass -e LIQ_DB_NAME="$TEST_DB" -e CHANGELOG_FORMAT=xml liquibase-test --init; then
...
EOF
    ...
    if docker compose -f test/docker-compose.yaml run --rm -e LIQ_DB_HOST=postgres-test -e LIQ_DB_USER=testuser -e LIQ_DB_PASSWORD=testpass -e LIQ_DB_NAME="$TEST_DB" -e LIQ_DB_SNAPSHOT="$REF_DB" -e CHANGELOG_FORMAT=xml -e LIQUIBASE_COMMAND_REFERENCE_URL="jdbc:postgresql://postgres-test:5432/$REF_DB" liquibase-test --generate; then
...
test_rollback_to_changeset() {
    ...
    if docker compose -f test/docker-compose.yaml run --rm -e LIQ_DB_HOST=postgres-test -e LIQ_DB_USER=testuser -e LIQ_DB_PASSWORD=testpass -e LIQ_DB_NAME="$TEST_DB" liquibase-test --rollback-to-changeset "migkit:add-product-fields"; then
